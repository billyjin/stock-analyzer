...
def get_sector_by_symbol(symbol: str) -> str:
    """주식 코드로 섹터 찾기"""
    for sector, stocks in KOREAN_SECTORS.items():
        if symbol in stocks:
            return sector
    return "기타"

def get_korean_stock_name(symbol: str) -> str:
    """주식 코드로 한국 주식명 찾기"""
    for sector, stocks in KOREAN_SECTORS.items():
        if symbol in stocks:
...
def is_small_cap_candidate(symbol: str) -> bool:
    """소형주/잡주 후보인지 확인"""
    small_cap_sectors = ["소형주_IT테크", "소형주_바이오", "소형주_게임", "테마주_신재생", "테마주_방산", "잡주_후보"]
    sector = get_sector_by_symbol(symbol)
    return sector in small_cap_sectors
